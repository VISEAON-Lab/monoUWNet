...
class Trainer:
    ...
    def train(self):
        ...
        reprojLossTotal = self.run_epoch()
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            reprojLossTotal += toNumpy(losses["loss/reprojLoss"].cpu())
        ...
        reprojLossTotal/=batch_idx
        print(f"tot reprojection loss {reprojLossTotal}")
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                if self.opt.get_gt_pose and inputs["T_curr2prev"] is not None:
                    if frame_id<0:
                        T = inputs["T_curr2prev"].type(torch.FloatTensor)
                    else:
                        T = inputs["T_curr2next"].type(torch.FloatTensor)
                    
                    depth = F.interpolate(
                    0.1*inputs["depth_gt"], [self.opt.height, self.opt.width], mode="nearest")
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            losses["loss/reprojLoss"] += to_optimise.mean()
...
if __name__ == "__main__":
    reprojLossTotal_vec=[]
    for i in range(1,15):
        opts.frame_ids = [0, -i, i]
        opts.scales=[0]
        trainer = Trainer(opts)
        reprojLossTotal = trainer.train()
        reprojLossTotal_vec.append(reprojLossTotal)
        print(f"reprojection loss for {opts.frame_ids} is {reprojLossTotal}")


    print(reprojLossTotal_vec)